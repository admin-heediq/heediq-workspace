...
echo ""
echo "Next steps:"
echo "  1. Configure AWS SSO profiles for the accounts you have access to:"
echo "     aws configure sso --profile heediq-dev     # most developers"
echo "     aws configure sso --profile heediq-shared  # infra owners only"
echo "     aws configure sso --profile heediq-staging # infra owners only"
echo "     aws configure sso --profile heediq-prod    # infra owners only"
echo "     (Use the IAM Identity Center start URL from the management account console.)"
echo ""
echo "  2. Start Claude from the workspace root:"
echo "     cd $WORKSPACE_ROOT && claude"
echo ""
echo "  Note: heediq-infra/scripts/setup.sh is an owner-only one-time operation"
echo "  (CDK bootstrap + OIDC roles). Do NOT run it unless setting up a fresh AWS org."
echo ""
